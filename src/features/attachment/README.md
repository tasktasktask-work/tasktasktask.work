...
| `shrink.ts` | ブラウザ側の圧縮と、大きさの検査。欄が二つあるので、書き写さずここに置く |
...
- **`ThreadAttachForm` の押しボタンを `noscript` の外へ出さない。**
  本文へ付ける欄は、選んだ時点で送る。押しボタンは JavaScript が動かないときのためだけにある。
  外へ出すと、動く環境で押しどころが二つになる
- **送信中に、ファイルの入力欄を DOM から外さない。**
  外すと `requestSubmit()` がフォームからファイルを拾えなくなる。
  差し替えるのは中の文字だけにする
- `AttachmentPicker` はコメントの投稿欄のためのものである。
  こちらは本文と添付を一つの送信で出すので、選んだ時点では送れない
